			...
			do {
				...
				do {
					...
					if (go == 10) {
						...
						ChangePlayerRequest request = ChangePlayerRequest.newBuilder().build();

						int response = stub.changePlayer(request).getNextPlayer();
						player = response;
						...
						try {
							PlayResultRequest request = PlayResultRequest.newBuilder()
									.setRow(row)
									.setColumn(column)
									.setPlayer(player)
									.build();

							play_res = stub.playResult(request).getResult();
